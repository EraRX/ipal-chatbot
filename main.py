...
from reportlab.platypus import (
    SimpleDocTemplate, Paragraph, Spacer, Image, ListFlowable,
...
def fetch_web_info_cached(query: str):
    result = []
    # Alleen gebruiken als CSV niets oplevert
    try:
...
def vind_best_passend_antwoord(vraag, systeem=None, subthema='alles'):
    try:
        vraag_lower = vraag.lower()

        if systeem in (None, '', 'alles'):
            df_scope = faq_df  # alles
        else:
            try:
                df_scope = faq_df.xs(systeem, level='Systeem', drop_level=False)
            except KeyError:
                logging.warning(f"Geen data voor systeem: {systeem}")
                return None
            if subthema and subthema != 'alles':
                try:
                    df_scope = df_scope.xs(subthema, level='Subthema', drop_level=False)
                except KeyError:
...
if 'mode' not in st.session_state:
    st.session_state.mode = None   # 'Exact' | 'DocBase' | 'Intern' | 'Algemeen'
...
def main():
    ...
    if not st.session_state.mode:
        ...
        if c1.button('ExactOnline', use_container_width=True):
            ...
            st.session_state.selected_module = None
            add_msg('assistant', 'Gekozen: ExactOnline')
            st.rerun()

        if c2.button('DocBase', use_container_width=True):
            st.session_state.mode = 'DocBase'
        ...
        if c3.button('Zoeken Intern', use_container_width=True):
            st.session_state.mode = 'Intern'      # CSV, alle systemen
            st.session_state.selected_product = None
            st.session_state.selected_module = 'alles'
            add_msg('assistant', 'Gekozen: Zoeken Intern (CSV, alle systemen)')
            st.rerun()

        if c4.button('Zoeken Algemeen', use_container_width=True):
            st.session_state.mode = 'Algemeen'    # direct AI/web
            st.session_state.selected_product = None
            st.session_state.selected_module = 'alles'
            add_msg('assistant', 'Gekozen: Zoeken Algemeen (AI/web)')
            st.rerun()

        render_chat()
        return

    # Module selectie alleen bij Exact/DocBase
    if st.session_state.mode in ['Exact', 'DocBase'] and not st.session_state.selected_module:
        opts = subthema_dict.get(st.session_state.selected_product or '', [])
    ...
    if mode == 'Algemeen':
        ...
        # CSV over ALLE systemen (geen module-selectie)
        antwoord = vind_best_passend_antwoord(vraag, systeem=None, subthema='alles')
        if antwoord:
            try:
                ...
                logging.error(f"Rewrite-fout: {str(e)}")
            add_msg('assistant', antwoord + f"\n\n{AI_INFO}")
            st.rerun()
            return
        # Niets gevonden → melding, geen AI/web in deze modus
        add_msg('assistant', 'Geen passend resultaat gevonden in de interne kennisbank (CSV). Probeer specifieker of kies **Zoeken Algemeen**.')
        st.rerun()
        return

    else:
        # Exact/DocBase → CSV binnen gekozen systeem (en subthema indien gekozen)
        syst = st.session_state.selected_product
        modu = st.session_state.selected_module or 'alles'
        antwoord = vind_best_passend_antwoord(vraag, systeem=syst, subthema=modu)
        if antwoord:
            try:
                antwoord = chatgpt_cached([
                    {'role': 'system', 'content': 'Herschrijf eenvoudig en vriendelijk.'},
        ...
        # CSV niets gevonden → AI/web-fallback
        with st.spinner('de IPAL Helpdesk zoekt het juiste antwoord…'):
            try:
                web_info = fetch_web_info_cached(vraag)
                if web_info:
