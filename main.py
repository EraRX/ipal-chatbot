...
# - Cascade sluit automatisch; antwoord verschijnt buiten de expander
# - “Vette onopgemaakte” bron verdwijnt; we tonen alleen de nette, simpele uitleg
# - PDF met banner/logo + Copy; PDF zonder AI-INFO
...
def enrich_with_simple(answer: str) -> str:
    # Vervang de ruwe bron door de eenvoudige uitleg → geen "vet onopgemaakt" blok
...
DEFAULT_STATE = {
    ...
    # Cascade weergave/closing
    "cascade_open": True,
    "cascade_answer": None,        # finale tekst zonder AI-INFO (voor PDF)
    "cascade_display": None,       # getoonde tekst met AI-INFO
    "cascade_label": None,         # label voor vraag/breadcrumb
}
...
def main():
    ...
    with st.expander("Liever de klassieke cascade openen?", expanded=st.session_state.get("cascade_open", True)):
        if faq_df is None or faq_df.empty:
            ...
            # 7) Antwoord of oplossing → bepaal en sluit cascade
            if sel_soort != "(Kies)":
                if step6.empty:
                    ...
                    # Nette uitleg (vervangt ruwe bron)
                    ...
                    # Wat we tonen (met AI-INFO)
                    display_ans = with_info(final_ans)

                    # Bewaar voor weergave buiten expander + actie-balk
                    st.session_state["cascade_answer"]  = final_ans
                    st.session_state["cascade_display"] = display_ans
                    st.session_state["cascade_label"]   = label
                    st.session_state["selected_image"]  = afbeelding if afbeelding else None

                    # Actie-balk informatie
                    st.session_state["actionbar"] = {
                        "question": label,
                        "content": display_ans,
                        "image": st.session_state.get("selected_image"),
                    ...
                    # Sluit de expander en rerun zodat antwoord buiten de expander verschijnt
                    st.session_state["cascade_open"] = False
                    st.rerun()

    # ── Antwoord uit cascade (buiten expander) ───────────────────────────────
    if st.session_state.get("cascade_display"):
        st.subheader("Antwoord")
        st.write(st.session_state["cascade_display"])
        # Afbeelding tonen (optioneel)
        ...
        if img:
            try:
                ...
                pass
        # Copy & PDF-knoppen
        _copy_button(st.session_state["cascade_display"], hashlib.md5(st.session_state["cascade_display"].encode("utf-8")).hexdigest()[:8])
        pdf = make_pdf(st.session_state["cascade_label"] or "Vraag", st.session_state["cascade_answer"] or "")
        st.download_button(
            "📄 Download PDF",
            data=pdf,
            file_name="antwoord.pdf",
            mime="application/pdf",
            key=f"cascade_pdf_{hash((st.session_state['cascade_label'] or '') + (st.session_state['cascade_answer'] or ''))}"
        )
