...
def load_faq(path: str = 'faq.xlsx') -> pd.DataFrame:
    ...
    try:
        ...
        if any(col not in df.columns for col in required):
            ...
            return pd.DataFrame(columns=['combined', 'Antwoord of oplossing'])
        # Zorg voor optionele kolom Afbeelding
...
# Bouw subthema-dict\ nsubthema_dict = {}
if not faq_df.empty:
...
for key in ['history','selected_product','selected_module','reset_triggered']:
    if key not in st.session_state:
        st.session_state[key] = [] if key=='history' else False if key=='reset_triggered' else None
...
def get_ai_answer(text):
    msgs=[{'role':'system','content':'You are IPAL Chatbox, a helpful Dutch helpdesk assistant.'}]
    ...
    try:
        res=openai.chat.completions.create(model=MODEL,messages=msgs,temperature=0.3,max_tokens=400)
        return res.choices[0].message.content.strip()
    except:
        return None

def get_answer(text):
    if not faq_df.empty and st.session_state.selected_module:
        sub=faq_df[faq_df['Subthema']==st.session_state.selected_module]
        pat=re.escape(text)
        match=sub[sub['combined'].str.contains(pat,case=False,na=False)]
        if not match.empty:
            row=match.iloc[0]
            ans=row['Antwoord of oplossing']; img=row.get('Afbeelding')
            try:
...
def main():
    ...
    if not st.session_state.selected_product:
        ...
        c1,c2=st.columns(2)
        if c1.button('DocBase',use_container_width=True): st.session_state.selected_product='DocBase';st.rerun()
        if c2.button('Exact',use_container_width=True): st.session_state.selected_product='Exact';st.rerun()
