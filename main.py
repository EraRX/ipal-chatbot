...
# - Topbar: alleen RESET (leeg ook de cache)
# - Geen sidebar meer
...
def make_pdf(question: str, answer: str) -> bytes:
    """PDF met banner 'logopdf.png' over volle contentbreedte (hoogte schaalt automatisch)."""
    ...
    if os.path.exists("logopdf.png"):
        try:
            ...
            banner._restrictSize(content_width, 10000)  # breedte = content_width, hoogte in verhouding
            banner.hAlign = "LEFT"
            story.append(banner)
...
def vind_best_algemeen_AI(vraag: str) -> str:
    """Internet: géén CSV. Alleen AI + één verhelderende vraag max (of suggesties)."""
    if client is None:
...
DEFAULT_STATE = {
    ...
    "last_processed_algemeen": "",   # debounce voor Internet
    # Conversatie-wizard
...
def _copy_button(text: str, key_suffix: str):
    """Werkende copy-knop met JS (via components.html) + fallback textarea (zonder f-string conflicts)."""
    payload = text or ""
...
def _detect_scope(msg: str) -> Optional[str]:
    ...
    if any(w in t for w in ["internet", "web", "algemeen", "overig", "anders", "ik weet het niet"]):
        return "Algemeen"  # intern heet 'Algemeen'; label tonen we als "Internet"
    return None
...
with st.container():
    ...
    if c3.button("Zoeken", key="wizard_search", use_container_width=True):  # i.p.v. “Zoeken Intern”
    ...
    if c4.button("Internet", key="wizard_internet", use_container_width=True):  # i.p.v. “Zoeken Algemeen”
    ...
    if step in ("greet", "ask_scope"):
        ...
        if scope_guess is None:
            # Probeer CSV match te vinden; zo niet → Internet
...
def main():
    ...
    if st.session_state.get("selected_product") == "Algemeen":
        ...
        # Zoekbalk bovenaan (zelfde hoogte/stijl als Zoeken)
        algemeen_vraag = st.text_input(
            " ",
            placeholder="Stel uw internetvraag:",
